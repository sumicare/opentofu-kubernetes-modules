...
[ ! -d "ome" ] && git clone --depth 1 --filter=blob:none --sparse https://github.com/sgl-project/ome
cd ome || exit 1
[ ! -d "charts" ] && git sparse-checkout set charts/ome-resources
cd charts/ome-resources || exit 1

cp -f ../../../values.yaml .

helm template . --namespace ome --values values.yaml 2>&1 |
  awk '
    BEGIN {
        skip_resource = 0
        resource_content = ""
...
helm template . --namespace ome --values values.yaml --include-crds --set crds.enabled=true 2>&1 |
  awk '
        BEGIN {
            include_resource = 0
            resource_content = ""
