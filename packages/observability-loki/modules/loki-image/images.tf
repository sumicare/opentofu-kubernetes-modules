...
resource "docker_image" "loki" {
  name         = "${var.repository}${var.org}/loki:${var.loki_version}"
  keep_locally = true
  triggers = {
  ...
  build {
    ...
    tag        = ["${var.repository}${var.org}/loki:${var.loki_version}"]
    build_args = {
      REPO : var.repository
      ORG : var.org
...
resource "docker_registry_image" "loki" {
  count         = var.registry_auth == null ? 0 : 1
  name          = docker_image.loki.name
  keep_remotely = true
