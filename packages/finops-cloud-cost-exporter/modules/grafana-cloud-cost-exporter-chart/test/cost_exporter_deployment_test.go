...
var _ = Describe("CostExporter Deployment", func() {
	var (
	...
	BeforeEach(func() {
		...
		namespace = "kube-system" // Default namespace for cost-exporter
	...
	Describe("Deployment validation", func() {
		DescribeTable("should validate deployment properties",
			...
			Entry("deployment exists", func(cs *kubernetes.Clientset, ctx context.Context, ns string) {
				deployment, err := cs.AppsV1().Deployments(ns).Get(ctx, "cost-exporter", metav1.GetOptions{})
				Expect(err).NotTo(HaveOccurred())
				Expect(deployment).NotTo(BeNil())
				Expect(deployment.ObjectMeta.Name).To(Equal("cost-exporter"))
			}),
		...
		It("should use correct cost-exporter version from package.json", func() {
			// Verify cost-exporter version is set
			Expect(terraformOptions.Vars).To(HaveKey("cost-exporter_version"))
			version := terraformOptions.Vars["cost-exporter_version"]
		...
		DescribeTable("should validate pod properties",
			...
			Entry("has pods running", func(cs *kubernetes.Clientset, ctx context.Context, ns string) {
				...
				pods, err := cs.CoreV1().Pods(ns).List(ctx, metav1.ListOptions{
					LabelSelector: "app.kubernetes.io/part-of=cost-exporter",
				})
				Expect(err).NotTo(HaveOccurred())
