...
resource "docker_image" "volcano" {
  name         = "${var.repository}${var.org}/volcano:${var.volcano_version}"
  keep_locally = true
  triggers = {
    Dockerfile         = filesha256("${path.module}/Dockerfile")
    DebianVersion      = var.debian_version
    VolcanoVersion = var.volcano_version
  }
  ...
  build {
    ...
    tag        = ["${var.repository}${var.org}/volcano:${var.volcano_version}"]
    build_args = {
      REPO : var.repository
      ORG : var.org
...
resource "docker_registry_image" "volcano" {
  count         = var.registry_auth == null ? 0 : 1
  name          = docker_image.volcano.name
  keep_remotely = true
  ...
  depends_on = [docker_image.volcano]
}
