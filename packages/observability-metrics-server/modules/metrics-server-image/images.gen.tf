...
resource "docker_image" "metrics_server" {
  name         = "${var.repository}${var.org}/metrics_server:${var.metrics_server_version}"
  keep_locally = true
  triggers = {
    Dockerfile         = filesha256("${path.module}/Dockerfile")
    DebianVersion      = var.debian_version
    MetricsServerVersion = var.metrics_server_version
  }
  ...
  build {
    ...
    tag        = ["${var.repository}${var.org}/metrics_server:${var.metrics_server_version}"]
    build_args = {
      REPO : var.repository
      ORG : var.org
...
resource "docker_registry_image" "metrics_server" {
  count         = var.registry_auth == null ? 0 : 1
  name          = docker_image.metrics_server.name
  keep_remotely = true
  ...
  depends_on = [docker_image.metrics_server]
}
