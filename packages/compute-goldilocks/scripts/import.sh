...
helm template kedacore/keda --namespace keda --values values.yaml |
  awk '
    BEGIN {
        skip_resource = 0
        resource_content = ""
...
helm template fairwinds-stable/goldilocks --namespace goldilocks --include-crds --set crds.enabled=true--values values.yaml 2>&1 |
  awk '
        BEGIN {
            include_resource = 0
            resource_content = ""
