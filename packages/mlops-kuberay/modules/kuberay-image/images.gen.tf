...
resource "docker_image" "kuberay" {
  name         = "${var.repository}${var.org}/kuberay:${var.kuberay_version}"
  keep_locally = true
  triggers = {
    Dockerfile         = filesha256("${path.module}/Dockerfile")
    DebianVersion      = var.debian_version
    KuberayVersion = var.kuberay_version
  }
  ...
  build {
    ...
    tag        = ["${var.repository}${var.org}/kuberay:${var.kuberay_version}"]
    build_args = {
      REPO : var.repository
      ORG : var.org
...
resource "docker_registry_image" "kuberay" {
  count         = var.registry_auth == null ? 0 : 1
  name          = docker_image.kuberay.name
  keep_remotely = true
  ...
  depends_on = [docker_image.kuberay]
}
