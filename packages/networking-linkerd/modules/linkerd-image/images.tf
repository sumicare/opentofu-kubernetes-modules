...
resource "docker_image" "linkerd" {
  name         = "${var.repository}${var.org}/linkerd:${var.linkerd_version}"
  keep_locally = true
  triggers = {
  ...
  build {
    ...
    tag        = ["${var.repository}${var.org}/linkerd:${var.linkerd_version}"]
    build_args = {
      REPO : var.repository
      ORG : var.org
...
resource "docker_registry_image" "linkerd" {
  count         = var.registry_auth == null ? 0 : 1
  name          = docker_image.linkerd.name
  keep_remotely = true
  ...
  depends_on = [docker_image.linkerd]
}
