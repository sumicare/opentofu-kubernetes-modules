...
resource "docker_image" "openbao" {
  name         = "${var.repository}${var.org}/openbao:${var.openbao_version}"
  keep_locally = true
  triggers = {
    Dockerfile         = filesha256("${path.module}/Dockerfile")
    DebianVersion      = var.debian_version
    OpenbaoVersion = var.openbao_version
  }
  ...
  build {
    ...
    tag        = ["${var.repository}${var.org}/openbao:${var.openbao_version}"]
    build_args = {
      REPO : var.repository
      ORG : var.org
...
resource "docker_registry_image" "openbao" {
  count         = var.registry_auth == null ? 0 : 1
  name          = docker_image.openbao.name
  keep_remotely = true
  ...
  depends_on = [docker_image.openbao]
}
