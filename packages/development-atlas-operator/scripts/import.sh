...
helm template atlas-operator oci://ghcr.io/ariga/charts/atlas-operator -f values.yaml |
  awk '
    BEGIN {
        skip_resource = 0
        resource_content = ""
    ...
    /^---$/ {
        if (!skip_resource && resource_content != "") {
...
helm template atlas-operator oci://ghcr.io/ariga/charts/atlas-operator --namespace atlas-operator --values values.yaml --include-crds --set crds.enabled=true 2>&1 |
  awk '
        BEGIN {
            include_resource = 0
            resource_content = ""
