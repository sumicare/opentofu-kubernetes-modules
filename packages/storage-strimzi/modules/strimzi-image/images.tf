...
resource "docker_image" "strimzi" {
  name         = "${var.repository}${var.org}/strimzi:${var.strimzi_version}"
  keep_locally = true
  triggers = {
    Dockerfile         = filesha256("${path.module}/Dockerfile")
    DebianVersion      = var.debian_version
    StrimziVersion = var.strimzi_version
  }
  ...
  build {
    ...
    tag        = ["${var.repository}${var.org}/strimzi:${var.strimzi_version}"]
    build_args = {
      REPO : var.repository
      ORG : var.org
...
resource "docker_registry_image" "strimzi" {
  count         = var.registry_auth == null ? 0 : 1
  name          = docker_image.strimzi.name
  keep_remotely = true
  ...
  depends_on = [docker_image.strimzi]
}
