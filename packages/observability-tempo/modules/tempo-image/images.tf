...
resource "docker_image" "tempo" {
  name         = "${var.repository}${var.org}/tempo:${var.tempo_version}"
  keep_locally = true
  triggers = {
    Dockerfile         = filesha256("${path.module}/Dockerfile")
    DebianVersion      = var.debian_version
    TempoVersion = var.tempo_version
  }
  ...
  build {
    ...
    tag        = ["${var.repository}${var.org}/tempo:${var.tempo_version}"]
    build_args = {
      REPO : var.repository
      ORG : var.org
...
resource "docker_registry_image" "tempo" {
  count         = var.registry_auth == null ? 0 : 1
  name          = docker_image.tempo.name
  keep_remotely = true
