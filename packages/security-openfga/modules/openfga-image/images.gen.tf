...
resource "docker_image" "openfga" {
  name         = "${var.repository}${var.org}/openfga:${var.openfga_version}"
  keep_locally = true
  triggers = {
    Dockerfile         = filesha256("${path.module}/Dockerfile")
    DebianVersion      = var.debian_version
    OpenfgaVersion = var.openfga_version
  }
  ...
  build {
    ...
    tag        = ["${var.repository}${var.org}/openfga:${var.openfga_version}"]
    build_args = {
      REPO : var.repository
      ORG : var.org
...
resource "docker_registry_image" "openfga" {
  count         = var.registry_auth == null ? 0 : 1
  name          = docker_image.openfga.name
  keep_remotely = true
  ...
  depends_on = [docker_image.openfga]
}
