...
resource "docker_image" "ome" {
  name         = "${var.repository}${var.org}/ome:${var.ome_version}"
  keep_locally = true
  triggers = {
    Dockerfile         = filesha256("${path.module}/Dockerfile")
    DebianVersion      = var.debian_version
    OmeVersion = var.ome_version
  }
  ...
  build {
    ...
    tag        = ["${var.repository}${var.org}/ome:${var.ome_version}"]
    build_args = {
      REPO : var.repository
      ORG : var.org
      DEBIAN_VERSION : var.debian_version
      OME_VERSION : var.ome_version
    }
    label = {
      author : var.org
    }
...
resource "docker_registry_image" "ome" {
  count         = var.registry_auth == null ? 0 : 1
  name          = docker_image.ome.name
  keep_remotely = true
