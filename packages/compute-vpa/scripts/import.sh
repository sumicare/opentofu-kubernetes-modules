...
helm template fairwinds-stable/vpa --namespace vpa --values values.yaml |
  awk '
    BEGIN {
        skip_resource = 0
        resource_content = ""
...
helm template fairwinds-stable/vpa --namespace vpa --values values.yaml --include-crds --set crds.enabled=true 2>&1 |
  awk '
        BEGIN {
            include_resource = 0
            resource_content = ""
