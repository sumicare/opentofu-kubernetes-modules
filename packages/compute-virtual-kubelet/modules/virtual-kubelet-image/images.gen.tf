...
resource "docker_image" "keda" {
  name         = "${var.repository}${var.org}/keda:${var.keda_version}"
  keep_locally = true
  triggers = {
    Dockerfile         = filesha256("${path.module}/Dockerfile")
    DebianVersion      = var.debian_version
    KedaVersion = var.keda_version
  }
  ...
  build {
    ...
    tag        = ["${var.repository}${var.org}/keda:${var.keda_version}"]
    build_args = {
      REPO : var.repository
      ORG : var.org
...
resource "docker_registry_image" "keda" {
  count         = var.registry_auth == null ? 0 : 1
  name          = docker_image.keda.name
  keep_remotely = true
