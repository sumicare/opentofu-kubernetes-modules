...
resource "docker_image" "reloader" {
  name         = "${var.repository}${var.org}/reloader:${var.reloader_version}"
  keep_locally = true
  triggers = {
    Dockerfile         = filesha256("${path.module}/Dockerfile")
    DebianVersion      = var.debian_version
    ReloaderVersion = var.reloader_version
  }
  ...
  build {
    ...
    tag        = ["${var.repository}${var.org}/reloader:${var.reloader_version}"]
    build_args = {
      REPO : var.repository
      ORG : var.org
...
resource "docker_registry_image" "reloader" {
  count         = var.registry_auth == null ? 0 : 1
  name          = docker_image.reloader.name
  keep_remotely = true
  ...
  depends_on = [docker_image.reloader]
}
