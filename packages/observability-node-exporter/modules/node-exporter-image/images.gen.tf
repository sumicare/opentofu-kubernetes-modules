...
resource "docker_image" "node_exporter" {
  name         = "${var.repository}${var.org}/node_exporter:${var.node_exporter_version}"
  keep_locally = true
  triggers = {
  ...
  build {
    ...
    tag        = ["${var.repository}${var.org}/node_exporter:${var.node_exporter_version}"]
    build_args = {
      REPO : var.repository
      ORG : var.org
...
resource "docker_registry_image" "node_exporter" {
  count         = var.registry_auth == null ? 0 : 1
  name          = docker_image.node_exporter.name
  keep_remotely = true
  ...
  depends_on = [docker_image.node_exporter]
}
