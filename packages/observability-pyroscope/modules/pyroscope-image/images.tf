...
resource "docker_image" "pyroscope" {
  name         = "${var.repository}${var.org}/pyroscope:${var.pyroscope_version}"
  keep_locally = true
  triggers = {
    Dockerfile         = filesha256("${path.module}/Dockerfile")
    DebianVersion      = var.debian_version
    PyroscopeVersion = var.pyroscope_version
  }
  ...
  build {
    ...
    tag        = ["${var.repository}${var.org}/pyroscope:${var.pyroscope_version}"]
    build_args = {
      REPO : var.repository
      ORG : var.org
...
resource "docker_registry_image" "pyroscope" {
  count         = var.registry_auth == null ? 0 : 1
  name          = docker_image.pyroscope.name
  keep_remotely = true
  ...
  depends_on = [docker_image.pyroscope]
}
