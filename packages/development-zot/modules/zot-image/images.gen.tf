...
resource "docker_image" "zot" {
  name         = "${var.repository}${var.org}/zot:${var.zot_version}"
  keep_locally = true
  triggers = {
  ...
  build {
    ...
    tag        = ["${var.repository}${var.org}/zot:${var.zot_version}"]
    build_args = {
      REPO : var.repository
      ORG : var.org
...
resource "docker_registry_image" "zot" {
  count         = var.registry_auth == null ? 0 : 1
  name          = docker_image.zot.name
  keep_remotely = true
