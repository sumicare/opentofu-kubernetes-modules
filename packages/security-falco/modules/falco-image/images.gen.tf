...
resource "docker_image" "falco" {
  name         = "${var.repository}${var.org}/falco:${var.falco_version}"
  keep_locally = true
  triggers = {
    Dockerfile         = filesha256("${path.module}/Dockerfile")
    DebianVersion      = var.debian_version
    FalcoVersion = var.falco_version
  }
  ...
  build {
    ...
    tag        = ["${var.repository}${var.org}/falco:${var.falco_version}"]
    build_args = {
      REPO : var.repository
      ORG : var.org
...
resource "docker_registry_image" "falco" {
  count         = var.registry_auth == null ? 0 : 1
  name          = docker_image.falco.name
  keep_remotely = true
