...
resource "docker_image" "cnpg" {
  name         = "${var.repository}${var.org}/cnpg:${var.cnpg_version}"
  keep_locally = true
  triggers = {
    Dockerfile         = filesha256("${path.module}/Dockerfile")
    DebianVersion      = var.debian_version
    CnpgVersion = var.cnpg_version
  }
  ...
  build {
    ...
    tag        = ["${var.repository}${var.org}/cnpg:${var.cnpg_version}"]
    build_args = {
      REPO : var.repository
      ORG : var.org
...
resource "docker_registry_image" "cnpg" {
  count         = var.registry_auth == null ? 0 : 1
  name          = docker_image.cnpg.name
  keep_remotely = true
