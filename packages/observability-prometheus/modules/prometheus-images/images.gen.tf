...
resource "docker_image" "prometheus" {
  name         = "${var.repository}${var.org}/prometheus:${var.prometheus_version}"
  keep_locally = true
  triggers = {
  ...
  build {
    ...
    tag        = ["${var.repository}${var.org}/prometheus:${var.prometheus_version}"]
    build_args = {
      REPO : var.repository
      ORG : var.org
...
resource "docker_registry_image" "prometheus" {
  count         = var.registry_auth == null ? 0 : 1
  name          = docker_image.prometheus.name
  keep_remotely = true
