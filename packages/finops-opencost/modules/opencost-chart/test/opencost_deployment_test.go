...
var _ = Describe("OpenCost Deployment", func() {
	var (
	...
	BeforeEach(func() {
		...
		namespace = "kube-system" // Default namespace for open-cost
	...
	Describe("Deployment validation", func() {
		DescribeTable("should validate deployment properties",
			...
			Entry("deployment exists", func(cs *kubernetes.Clientset, ctx context.Context, ns string) {
				deployment, err := cs.AppsV1().Deployments(ns).Get(ctx, "open-cost", metav1.GetOptions{})
				Expect(err).NotTo(HaveOccurred())
				Expect(deployment).NotTo(BeNil())
				Expect(deployment.ObjectMeta.Name).To(Equal("open-cost"))
			}),
		...
		It("should use correct open-cost version from package.json", func() {
			// Verify open-cost version is set
			Expect(terraformOptions.Vars).To(HaveKey("open-cost_version"))
			version := terraformOptions.Vars["open-cost_version"]
		...
		DescribeTable("should validate pod properties",
			...
			Entry("has pods running", func(cs *kubernetes.Clientset, ctx context.Context, ns string) {
				...
				pods, err := cs.CoreV1().Pods(ns).List(ctx, metav1.ListOptions{
					LabelSelector: "app.kubernetes.io/part-of=open-cost",
				})
				Expect(err).NotTo(HaveOccurred())
