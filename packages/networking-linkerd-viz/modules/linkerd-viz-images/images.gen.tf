...
resource "docker_image" "linkerd_viz" {
  name         = "${var.repository}${var.org}/linkerd_viz:${var.linkerd_viz_version}"
  keep_locally = true
  triggers = {
    Dockerfile         = filesha256("${path.module}/Dockerfile")
    DebianVersion      = var.debian_version
    LinkerdVizVersion = var.linkerd_viz_version
  }
  ...
  build {
    ...
    tag        = ["${var.repository}${var.org}/linkerd_viz:${var.linkerd_viz_version}"]
    build_args = {
      REPO : var.repository
      ORG : var.org
...
resource "docker_registry_image" "linkerd_viz" {
  count         = var.registry_auth == null ? 0 : 1
  name          = docker_image.linkerd_viz.name
  keep_remotely = true
  ...
  depends_on = [docker_image.linkerd_viz]
}
