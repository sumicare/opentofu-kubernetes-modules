...
resource "docker_image" "minio" {
  name         = "${var.repository}${var.org}/minio:${var.minio_version}"
  keep_locally = true
  triggers = {
    Dockerfile         = filesha256("${path.module}/Dockerfile")
    DebianVersion      = var.debian_version
    MinioVersion = var.minio_version
  }
  ...
  build {
    ...
    tag        = ["${var.repository}${var.org}/minio:${var.minio_version}"]
    build_args = {
      REPO : var.repository
      ORG : var.org
...
resource "docker_registry_image" "minio" {
  count         = var.registry_auth == null ? 0 : 1
  name          = docker_image.minio.name
  keep_remotely = true
