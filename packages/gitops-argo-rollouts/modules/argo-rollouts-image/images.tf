...
resource "docker_image" "argo_rollouts" {
  name         = "${var.repository}${var.org}/argo_rollouts:${var.argo_rollouts_version}"
  keep_locally = true
  triggers = {
    Dockerfile         = filesha256("${path.module}/Dockerfile")
    DebianVersion      = var.debian_version
    ArgoRolloutsVersion = var.argo_rollouts_version
  }
  ...
  build {
    ...
    tag        = ["${var.repository}${var.org}/argo_rollouts:${var.argo_rollouts_version}"]
    build_args = {
      REPO : var.repository
      ORG : var.org
...
resource "docker_registry_image" "argo_rollouts" {
  count         = var.registry_auth == null ? 0 : 1
  name          = docker_image.argo_rollouts.name
  keep_remotely = true
  ...
  depends_on = [docker_image.argo_rollouts]
}
