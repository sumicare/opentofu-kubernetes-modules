...
resource "docker_image" "mongodb" {
  name         = "${var.repository}${var.org}/mongodb:${var.mongodb_version}"
  keep_locally = true
  triggers = {
  ...
  build {
    ...
    tag        = ["${var.repository}${var.org}/mongodb:${var.mongodb_version}"]
    build_args = {
      REPO : var.repository
      ORG : var.org
...
resource "docker_registry_image" "mongodb" {
  count         = var.registry_auth == null ? 0 : 1
  name          = docker_image.mongodb.name
  keep_remotely = true
  ...
  depends_on = [docker_image.mongodb]
}
