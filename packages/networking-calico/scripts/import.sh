...
# Extract ConfigMaps separately (k2tf truncates data sections, use tfk8s instead)
helm template calico/tigera-operator --namespace calico --values values.yaml 2>&1 |
  awk '
    BEGIN {
        include_resource = 0
        resource_content = ""
    ...
    /^kind:/ {
        ...
        # Include only ConfigMaps
        if (current_kind == "ConfigMap" && current_api_version == "v1") {
            include_resource = 1
            resource_content = "---\n" resource_content
        }
        next
    ...
    }' | tfk8s --strip >../configmap.tf

# Extract all other standard k8s resources (excluding ConfigMaps)
helm template calico/tigera-operator --namespace argocd --values values.yaml 2>&1 |
  awk '
    BEGIN {
        skip_resource = 0
        resource_content = ""
