...
resource "docker_image" "dex" {
  ...
  build {
    ...
    tag        = ["${var.repository}${var.org}/dex:${var.dex_version}"]
    build_args = {
      REPO : var.repository
      ORG : var.org
...
resource "docker_registry_image" "dex" {
  count         = var.registry_auth == null ? 0 : 1
  name          = docker_image.dex.name
  keep_remotely = true
