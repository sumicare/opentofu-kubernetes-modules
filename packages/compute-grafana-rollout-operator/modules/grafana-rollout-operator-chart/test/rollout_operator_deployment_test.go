...
var _ = Describe("RolloutOperator Deployment", func() {
	var (
	...
	BeforeEach(func() {
		...
		namespace = "kube-system" // Default namespace for rollout-operator
	...
	Describe("Deployment validation", func() {
		DescribeTable("should validate deployment properties",
			...
			Entry("deployment exists", func(cs *kubernetes.Clientset, ctx context.Context, ns string) {
				deployment, err := cs.AppsV1().Deployments(ns).Get(ctx, "rollout-operator", metav1.GetOptions{})
				Expect(err).NotTo(HaveOccurred())
				Expect(deployment).NotTo(BeNil())
				Expect(deployment.ObjectMeta.Name).To(Equal("rollout-operator"))
			}),
		...
		It("should use correct rollout-operator version from package.json", func() {
			// Verify rollout-operator version is set
			Expect(terraformOptions.Vars).To(HaveKey("rollout_operator_version"))
			version := terraformOptions.Vars["rollout_operator_version"]
		...
		DescribeTable("should validate pod properties",
			...
			Entry("has pods running", func(cs *kubernetes.Clientset, ctx context.Context, ns string) {
				...
				pods, err := cs.CoreV1().Pods(ns).List(ctx, metav1.ListOptions{
					LabelSelector: "app.kubernetes.io/part-of=rollout-operator",
				})
				Expect(err).NotTo(HaveOccurred())
