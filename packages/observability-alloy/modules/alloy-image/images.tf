...
resource "docker_image" "alloy" {
  name         = "${var.repository}${var.org}/alloy:${var.alloy_version}"
  keep_locally = true
  triggers = {
    Dockerfile         = filesha256("${path.module}/Dockerfile")
    DebianVersion      = var.debian_version
    AlloyVersion = var.alloy_version
  }
  ...
  build {
    ...
    tag        = ["${var.repository}${var.org}/alloy:${var.alloy_version}"]
    build_args = {
      REPO : var.repository
      ORG : var.org
...
resource "docker_registry_image" "alloy" {
  count         = var.registry_auth == null ? 0 : 1
  name          = docker_image.alloy.name
  keep_remotely = true
