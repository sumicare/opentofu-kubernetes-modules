...
resource "docker_image" "argo_workflows" {
  name         = "${var.repository}${var.org}/argo_workflows:${var.argo_workflows_version}"
  keep_locally = true
  triggers = {
    Dockerfile         = filesha256("${path.module}/Dockerfile")
    DebianVersion      = var.debian_version
    ArgoWorkflowsVersion = var.argo_workflows_version
  }
  ...
  build {
    ...
    tag        = ["${var.repository}${var.org}/argo_workflows:${var.argo_workflows_version}"]
    build_args = {
      REPO : var.repository
      ORG : var.org
...
resource "docker_registry_image" "argo_workflows" {
  count         = var.registry_auth == null ? 0 : 1
  name          = docker_image.argo_workflows.name
  keep_remotely = true
  ...
  depends_on = [docker_image.argo_workflows]
}
