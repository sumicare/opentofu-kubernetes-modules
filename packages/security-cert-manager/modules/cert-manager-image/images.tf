...
resource "docker_image" "cert_manager" {
  name         = "${var.repository}${var.org}/cert_manager:${var.cert_manager_version}"
  keep_locally = true
  triggers = {
  ...
  build {
    ...
    tag        = ["${var.repository}${var.org}/cert_manager:${var.cert_manager_version}"]
    build_args = {
      REPO : var.repository
      ORG : var.org
...
resource "docker_registry_image" "cert_manager" {
  count         = var.registry_auth == null ? 0 : 1
  name          = docker_image.cert_manager.name
  keep_remotely = true
  ...
  depends_on = [docker_image.cert_manager]
}
