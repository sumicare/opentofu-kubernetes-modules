...
helm template volcano/volcano --namespace volcano --values values.yaml --create-namespace 2>&1 |
  awk '
    BEGIN {
        skip_resource = 0
        resource_content = ""
...
helm template volcano/volcano --namespace volcano --values values.yaml --include-crds --set crds.enabled=true 2>&1 |
  awk '
        BEGIN {
            include_resource = 0
            resource_content = ""
