...
resource "docker_image" "calico" {
  name         = "${var.repository}${var.org}/calico:${var.calico_version}"
  keep_locally = true
  triggers = {
    Dockerfile         = filesha256("${path.module}/Dockerfile")
    DebianVersion      = var.debian_version
    CalicoVersion = var.calico_version
  }
  ...
  build {
    ...
    tag        = ["${var.repository}${var.org}/calico:${var.calico_version}"]
    build_args = {
      REPO : var.repository
      ORG : var.org
...
resource "docker_registry_image" "calico" {
  count         = var.registry_auth == null ? 0 : 1
  name          = docker_image.calico.name
  keep_remotely = true
  ...
  depends_on = [docker_image.calico]
}
