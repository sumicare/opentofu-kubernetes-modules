...
resource "docker_image" "theia" {
  name         = "${var.repository}${var.org}/theia:${var.theia_version}"
  keep_locally = true
  triggers = {
    Dockerfile         = filesha256("${path.module}/Dockerfile")
    DebianVersion      = var.debian_version
    TheiaVersion = var.theia_version
  }
  ...
  build {
    ...
    tag        = ["${var.repository}${var.org}/theia:${var.theia_version}"]
    build_args = {
      REPO : var.repository
      ORG : var.org
      DEBIAN_VERSION : var.debian_version
      THEIA_VERSION : var.theia_version
    }
    label = {
      author : var.org
    }
...
resource "docker_registry_image" "theia" {
  count         = var.registry_auth == null ? 0 : 1
  name          = docker_image.theia.name
  keep_remotely = true
