...
resource "docker_image" "grafana_mcp" {
  name         = "${var.repository}${var.org}/grafana_mcp:${var.grafana_mcp_version}"
  keep_locally = true
  triggers = {
    Dockerfile         = filesha256("${path.module}/Dockerfile")
    DebianVersion      = var.debian_version
    GrafanaMcpVersion = var.grafana_mcp_version
  }
  ...
  build {
    ...
    tag        = ["${var.repository}${var.org}/grafana_mcp:${var.grafana_mcp_version}"]
    build_args = {
      REPO : var.repository
      ORG : var.org
      DEBIAN_VERSION : var.debian_version
      GRAFANA_MCP_VERSION : var.grafana_mcp_version
    }
    label = {
      author : var.org
    }
...
resource "docker_registry_image" "grafana_mcp" {
  count         = var.registry_auth == null ? 0 : 1
  name          = docker_image.grafana_mcp.name
  keep_remotely = true
  ...
  depends_on = [docker_image.grafana_mcp]
}
