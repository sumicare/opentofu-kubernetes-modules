...
resource "docker_image" "velero" {
  name         = "${var.repository}${var.org}/velero:${var.velero_version}"
  keep_locally = true
  triggers = {
    Dockerfile         = filesha256("${path.module}/Dockerfile")
    DebianVersion      = var.debian_version
    VeleroVersion = var.velero_version
  }
  ...
  build {
    ...
    tag        = ["${var.repository}${var.org}/velero:${var.velero_version}"]
    build_args = {
      REPO : var.repository
      ORG : var.org
...
resource "docker_registry_image" "velero" {
  count         = var.registry_auth == null ? 0 : 1
  name          = docker_image.velero.name
  keep_remotely = true
  ...
  depends_on = [docker_image.velero]
}
