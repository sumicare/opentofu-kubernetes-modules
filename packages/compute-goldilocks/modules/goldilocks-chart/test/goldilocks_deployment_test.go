...
var _ = Describe("Goldilocks Deployment", func() {
	var (
	...
	BeforeEach(func() {
		...
		namespace = "kube-system" // Default namespace for goldilocks
	...
	Describe("Deployment validation", func() {
		DescribeTable("should validate deployment properties",
			...
			Entry("deployment exists", func(cs *kubernetes.Clientset, ctx context.Context, ns string) {
				deployment, err := cs.AppsV1().Deployments(ns).Get(ctx, "goldilocks", metav1.GetOptions{})
				Expect(err).NotTo(HaveOccurred())
				Expect(deployment).NotTo(BeNil())
				Expect(deployment.ObjectMeta.Name).To(Equal("goldilocks"))
			}),
		...
		It("should use correct goldilocks version from package.json", func() {
			// Verify goldilocks version is set
			Expect(terraformOptions.Vars).To(HaveKey("goldilocks_version"))
			version := terraformOptions.Vars["goldilocks_version"]
		...
		DescribeTable("should validate pod properties",
			...
			Entry("has pods running", func(cs *kubernetes.Clientset, ctx context.Context, ns string) {
				...
				pods, err := cs.CoreV1().Pods(ns).List(ctx, metav1.ListOptions{
					LabelSelector: "app.kubernetes.io/part-of=goldilocks",
				})
				Expect(err).NotTo(HaveOccurred())
