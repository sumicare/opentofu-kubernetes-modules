...
resource "docker_image" "mimir" {
  name         = "${var.repository}${var.org}/mimir:${var.mimir_version}"
  keep_locally = true
  triggers = {
    Dockerfile         = filesha256("${path.module}/Dockerfile")
    DebianVersion      = var.debian_version
    MimirVersion = var.mimir_version
  }
  ...
  build {
    ...
    tag        = ["${var.repository}${var.org}/mimir:${var.mimir_version}"]
    build_args = {
      REPO : var.repository
      ORG : var.org
...
resource "docker_registry_image" "mimir" {
  count         = var.registry_auth == null ? 0 : 1
  name          = docker_image.mimir.name
  keep_remotely = true
