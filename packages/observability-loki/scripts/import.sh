...
helm template grafana/loki --namespace loki --api-versions autoscaling.k8s.io/v1 --api-versions autoscaling/v2 --api-versions keda.sh/v1alpha1 --values values.yaml 2>&1 |
  awk '
    BEGIN {
        skip_resource = 0
        resource_content = ""
