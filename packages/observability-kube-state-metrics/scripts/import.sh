...
helm template oci://ghcr.io/prometheus-community/charts/kube-state-metrics --namespace kube-state-metrics --values values.yaml 2>&1 |
  awk '
    BEGIN {
        skip_resource = 0
        resource_content = ""
...
helm template oci://ghcr.io/prometheus-community/charts/kube-state-metrics --namespace kube-state-metrics --values values.yaml 2>&1 |
  awk '
        ...
        /^kind:/ {
            ...
            unsupported_kinds = "CustomResourceDefinition|BackendTLSPolicy|Certificate|ExternalSecret|GRPCRoute|HTTPRoute|ServiceMonitor|CiliumNetworkPolicy|Issuer|ClusterIssuer|CertificateRequest"
